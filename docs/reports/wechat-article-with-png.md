...
>
> 说明：本文以 **PNG 图片**方式插图，文件需放在 `docs/reports/diagrams/`，且与 `.drawio` **同名**（例如 `01-project-goals-pyramid.drawio` → `01-project-goals-pyramid.png`）。
...
图1：项目目标金字塔（从“协作可交付”到“功能/性能/安全”）

![图1：项目目标金字塔](diagrams/01-project-goals-pyramid.png)
...
图2：10 个 Agent 的组织与分工边界（可复制的协作结构）

![图2：多Agent团队架构](diagrams/02-agent-team-architecture.png)
...
![图3：每日双决策窗口](diagrams/03-daily-decision-windows.png)
...
图4：系统总体架构（从前端到网关到服务与数据）

![图4：系统总体架构](diagrams/04-system-architecture.png)

图5：服务交互矩阵（谁依赖谁、如何避免循环依赖）

![图5：服务交互矩阵](diagrams/05-service-interaction-matrix.png)
...
图6：转账/关键链路时序（用链路图把“合约”落到请求级）

![图6：转账关键链路时序](diagrams/08-transfer-sequence.png)
...
图7：风控引擎流程（规则、评分、拦截与证据链）

![图7：风控引擎流程](diagrams/09-risk-engine-flow.png)

图8：安全分层（从输入验证到授权、审计与脱敏）

![图8：安全分层](diagrams/15-security-layers.png)
...
![图9：测试金字塔](diagrams/12-test-pyramid.png)
...
## 附：本文引用的图片文件清单（与 drawio 同名）

> 说明：当前仓库中 `docs/reports/diagrams/` 仅有 `.drawio` 源文件；请将其导出为同名 `.png` 并放在同目录，本文图片即可正常显示。

- `docs/reports/diagrams/01-project-goals-pyramid.png`
- `docs/reports/diagrams/02-agent-team-architecture.png`
- `docs/reports/diagrams/03-daily-decision-windows.png`
- `docs/reports/diagrams/04-system-architecture.png`
- `docs/reports/diagrams/05-service-interaction-matrix.png`
- `docs/reports/diagrams/08-transfer-sequence.png`
- `docs/reports/diagrams/09-risk-engine-flow.png`
- `docs/reports/diagrams/12-test-pyramid.png`
- `docs/reports/diagrams/15-security-layers.png`
